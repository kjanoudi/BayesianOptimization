...
class SequentialDomainReductionTransformer(DomainTransformer):
    ...
    def initialize(self, target_space: TargetSpace) -> None:
        ...
        print(f"Init: Target Space.Bounds: {target_space.bounds} ({type(target_space.bounds)})")
        self.original_bounds = np.copy(target_space.bounds)
        self.bounds = [self.original_bounds]
        print(f"Init: original_bounds: {self.original_bounds} ({type(self.original_bounds)})")

        self.previous_optimal = np.mean(target_space.bounds, axis=1)
        self.current_optimal = np.mean(target_space.bounds, axis=1)
        print(f"Init: previous_optimal: {self.previous_optimal} ({type(self.previous_optimal)})")
        print(f"Init: current_optimal: {self.current_optimal} ({type(self.current_optimal)})")
        self.r = target_space.bounds[:, 1] - target_space.bounds[:, 0]
        print(f"Init: self.r: {self.r} ({type(self.r)})")
        self.previous_d = 2.0 * \
            (self.current_optimal - self.previous_optimal) / self.r
        print(f"Init: previous_d: {self.previous_d} ({type(self.previous_d)})")
        self.current_d = 2.0 * (self.current_optimal -
                                self.previous_optimal) / self.r

        print(f"Init: current_d: {self.current_d} ({type(self.current_d)})")
        self.c = self.current_d * self.previous_d
        print(f"Init: c: {self.c} ({type(self.c)})")
        self.c_hat = np.sqrt(np.abs(self.c)) * np.sign(self.c)
        print(f"Init: c_hat: {self.c_hat} ({type(self.c_hat)})")

        self.gamma = 0.5 * (self.gamma_pan * (1.0 + self.c_hat) +
                            self.gamma_osc * (1.0 - self.c_hat))
        print(f"Init: gamma: {self.gamma} ({type(self.gamma)})")

        self.contraction_rate = self.eta + \
            np.abs(self.current_d) * (self.gamma - self.eta)
        print(f"Init: contraction_rate: {self.contraction_rate} ({type(self.contraction_rate)})")
